...
class TestFiniteDifferenceMethods(unittest.TestCase):
    ...
    def test_dxdy(self):
        ...
        def dxdy_f1(x, y, m=1, n=1):
            return m*n*np.sin(m*x)*np.sin(n*y)
        ...
        for n in n_list:
            for length in length_list:
                ...
                error = np.abs( dxdy_f1(X, Y) - fd.dxdy(f1(X, Y), h) )
                rel_error = error/np.mean(np.abs(dxdy_f1(X, Y)))
    ...
    def test_dx2_dy2(self):
        ...
        def f1(x, y, m=1, n=1):
            return np.cos(m*x)*np.cos(n*y)
        def dx2_dy2_f1(x, y, m=1, n=1):
            return -(m**2 - n**2)*np.cos(m*x)*np.cos(n*y)
        
        n_list = [100, 1000]
        length_list = [2*np.pi, 4*np.pi, 16*np.pi]
        ...
        for n in n_list:
            for length in length_list:
                ...
                error = np.abs( dx2_dy2_f1(X, Y, m=2) 
                                - fd.dx2_dy2(f1(X, Y, m=2), h) )
                rel_error = error/np.mean(np.abs(dx2_dy2_f1(X, Y, m=2)))
