...
\begin{document}
	...
	\section{Active Nematic Fluid Equation}
	For active nematics, the active stress tensor is given by:
	\begin{equation}
		\sigma^a_{ij} = \zeta Q_{ij}
	\end{equation}
	To calculate the equation of motion from this stress tensor, we need to take the divergence:
	\begin{equation}
		\partial_j \sigma^a_{ji} = \zeta \partial_j Q_{ji}
	\end{equation}
	Now we take the curl to get rid of the pressure in the equation of motion:
	\begin{equation}
		\epsilon_{kli} \partial_l \partial_j \sigma^a_{ij} = \epsilon_{kli} \zeta \partial_l \partial_j Q_{ji}
	\end{equation}
	This then becomes the source term of the biharmonic equation in the stream function:
	\begin{equation}
		\beta_4 \partial^4 \psi_k = \zeta \epsilon_{kli} \partial_l \partial_j Q_{ji}
	\end{equation}
	Plugging in with the auxiliary variables yields:
	\begin{equation}
		\beta_4 \partial^4 \psi 
		= \zeta \left[ 
		\left( \partial_x^2 - \partial_y^2 \right) \nu
		+ \partial_x \partial_y \mu
		- \partial_x \partial_y \eta
		\right]
	\end{equation}
	Making this non-dimensional:
	\begin{equation}
		\overline{\psi} = \frac{\zeta}{\beta_4 \tau}
		\left[
		\left( \partial_x^2 - \partial_y^2 \right) \nu
		+ \partial_x \partial_y \mu
		- \partial_x \partial_y \eta
		\right]
	\end{equation}
	where all the derivatives are dimensionless now. Define the variable:
	\begin{equation}
		\overline{\zeta} \equiv \frac{\zeta}{\beta_4 \tau}
	\end{equation}
	So that the completely nondimensional equation reads:
	\begin{equation}
